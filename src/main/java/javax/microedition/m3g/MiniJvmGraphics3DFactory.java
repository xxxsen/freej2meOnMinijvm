...
import static javax.microedition.m3g.MiniJvmGraphics3DFactory.GlRenderer.READBACK_BYTES_PER_PIXEL;
...
{
	private static final String BGRA_READBACK_PROPERTY = "freej2me.m3g.readback.bgra";
	...
	private static final class MiniJvmGlBackend implements Graphics3D.SkinningBackend
	{
		...
		public void clear(Background background)
		{
			...
			frame.forceSoftware = !supportsBackground(background);
		...
		public void render(Mesh mesh, int submeshIndex, VertexBuffer vertices, TriangleStripArray triangles, Appearance appearance, Transform transform)
		{
			...
			else if (!supportsRenderItem(item))
			{
				frame.forceSoftware = true;
				if (frame.fallbackReason == null)
				{
		...
		{
			if (frame.softwarePassthrough || !glAvailable || frame.forceSoftware || !frame.clearCalled)
			{
				return false;
			}
			RenderItem item = RenderItem.createSkinned(mesh, triangles, appearance, copyTransform(transform));
			if (!supportsRenderItem(item))
			{
				return false;
			}
		...
		{
			return background == null || supportsBackgroundImage(background);
		}
		...
		private boolean supportsRenderItem(RenderItem item)
		{
			...
			if (item.appearance.getFog() != null)
			{
				int fogMode = item.appearance.getFog().getMode();
		...
		private boolean supportsTexture(Texture2D texture)
		{
			...
			return image != null && (image.getFormat() == Image2D.RGB || image.getFormat() == Image2D.RGBA);
		}
		...
		private boolean supportsBackgroundImage(Background background)
		{
			...
			return image.getFormat() == Image2D.RGB || image.getFormat() == Image2D.RGBA;
		}
	}
	...
	static final class GlRenderer
	{
		...
		{
			return resolveTargetSurface(target) != null;
		}

		void renderFrame(Graphics3D owner, FrameState frame)
		{
			RenderTargetSurface target = resolveTargetSurface(owner.getTarget());
			if (target == null)
			{
		...
		private void applyBackground(Graphics3D owner, Background background)
		{
			...
			boolean clearDepth = owner.isDepthBufferEnabled() && (background == null || background.isDepthClearEnabled());
		...
		private void renderItem(Graphics3D owner, RenderItem item)
		{
			...
			applyAppearance(owner, item.appearance);
		...
		private void applyAppearance(Graphics3D owner, Appearance appearance)
		{
			...
			glDepthMask(compositingMode.isDepthWriteEnabled() ? GL_TRUE : GL_FALSE);
		...
		private void applyTextures(Appearance appearance, boolean[] texturedUnits)
		{
			for (int unit = 0; unit < TEXTURE_UNIT_COUNT; unit++)
			{
				...
				int texId = configureTexture(unit, image,
		...
		private int configureTexture(int unit, Image2D image, int wrapS, int wrapT, int filter)
		{
			glActiveTexture(textureConstant(unit));
			...
			if (cached == null || image.isMutable())
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, image.getFormat() == Image2D.RGB ? GL_RGB : GL_RGBA,
						image.getWidth(), image.getHeight(), 0,
						image.getFormat() == Image2D.RGB ? GL_RGB : GL_RGBA,
						GL_UNSIGNED_BYTE, image.getImageData(), 0);
		...
		private void renderBackgroundImage(Graphics3D owner, Background background)
		{
			...
			uploadBackgroundTexture(texture,
			...
			glDisable(GL_POLYGON_OFFSET_FILL);
			if (texture.format == Image2D.RGBA)
			{
				glEnable(GL_BLEND);
				glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
			}
		...
		private boolean supportsNativeBufferedImageReadback()
		{
			...
			{
				String requested = System.getProperty(BGRA_READBACK_PROPERTY, "true");
				nativeBufferedImageReadbackSupported = Boolean.valueOf(Boolean.parseBoolean(requested));
			}
		...
		private RenderTargetSurface resolveTargetSurface(Object target)
		{
			...
			return image != null ? new BufferedImageTargetSurface(image, true) : null;
		}
		...
		private BackgroundTextureSource buildBackgroundTextureSource(Background background)
		{
			...
			boolean needsAlpha = image.getFormat() == Image2D.RGBA
			...
			int sourceBytesPerPixel = image.getFormat() == Image2D.RGBA ? 4 : 3;
			...
			for (int y = 0; y < cropHeight; y++)
			{
				...
				for (int x = 0; x < cropWidth; x++)
				{
					...
					int src = (sampleY * image.getWidth() + sampleX) * sourceBytesPerPixel;
					data[dst] = source[src];
					data[dst + 1] = source[src + 1];
					data[dst + 2] = source[src + 2];
					if (format == Image2D.RGBA)
					{
						data[dst + 3] = sourceBytesPerPixel == 4 ? source[src + 3] : (byte) 0xFF;
					}
				}
			}
		...
		private void uploadBackgroundTexture(BackgroundTextureSource texture, int wrapS, int wrapT, int filter)
		{
			...
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, filter);
			glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, filter);
			glTexImage2D(GL_TEXTURE_2D, 0, texture.format == Image2D.RGB ? GL_RGB : GL_RGBA,
					texture.width, texture.height, 0,
					texture.format == Image2D.RGB ? GL_RGB : GL_RGBA,
	...
	private static final class BackgroundTextureSource
	{
		...
		BackgroundTextureSource(int width, int height, int format, byte[] data)
		{
			this.width = width;
			this.height = height;
			this.format = format;
